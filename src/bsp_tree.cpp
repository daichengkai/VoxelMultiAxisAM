...
void BSPTree::BSPTreeFromMesh(MyMesh *mesh, int max_level_allowed, bool orthogonal_clipping) {

    for (size_t i = 0; i < mesh->faces.rows(); ++i) {
        Eigen::Vector3d p1, p2, p3;
        p1 = mesh->vectices.row(mesh->faces(i, 0));
        p2 = mesh->vectices.row(mesh->faces(i, 1));
        p3 = mesh->vectices.row(mesh->faces(i, 2));
...
bool
BSPTree::ComputePlaneEquation(Eigen::Vector3d &p1, Eigen::Vector3d &p2, Eigen::Vector3d &p3, Eigen::Vector4d &plane) {
...
void BSPTree::MeshTrimming(MyMesh *mesh){
    std::vector<TriangleForBSP> mesh_soup;
    mesh_soup.resize(mesh->faces.rows());
